...
class DemoMotion{
	...
	DemoMotion(String info, PVector start, PVector end, int motion, int steps, int speed){
		this.info = info;
		setOrigin(start);
		setTarget(end);
		setMover(motion, steps, speed);
	}
	...
	void setMover(int motion, int steps, int speed){
		this.m = new Mover(this.a.pos, this.b.pos);
		this.m.setMotion(motion, steps, 0);
