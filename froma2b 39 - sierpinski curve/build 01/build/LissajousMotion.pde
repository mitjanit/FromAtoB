...
class LissajousMotion extends Motion {
	...
	void calculatePath1(){
		...
		ratio = 10;  // Parells: 4, 6, 8, 10, 12, ...
		phase = 5*PI;  // 5*PI
		float k = ratio*2.0;
		float minAngle = (PI + PI/k)*direction;  // PI/8
		float maxAngle = (TWO_PI - PI/k)*direction;    // PI/8

		float b = getDistance()/1;  // ALTURA
		...
		for(int n=0; n<=nSteps; n++){
			...
			float y = center.y + a/(ratio/PI) + b*sin(angle);   // b/2.65
